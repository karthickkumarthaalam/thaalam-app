document.addEventListener("DOMContentLoaded", function () {
  ...
  if (likeBtn) {
    likeBtn.addEventListener("click", () =>
      handleLike(likeBtn, likeCountElem, podcastId, memberId)
    );
  }
});
...
function fetchRelatedPodcasts(name, excludedId) {
  ...
  fetch(
    `${window.API_BASE_URL}/podcasts?page=1&search=${encodeURIComponent(
      name
    )}&limit=4`
  )
    .then((res) => res.json())
...
function fetchComments(podcastId, page = 1) {
  ...
  fetch(
    `${window.API_BASE_URL}/podcasts/${podcastId}/comments?page=${page}&limit=${COMMENTS_LIMIT}`
  )
    .then((res) => res.json())
    .then((result) => {
...
function renderComments(comments) {
  ...
  comments.forEach((comment) => {
    ...
    commentDiv.innerHTML = `
            ...
            } <span>• ${timeAgo(comment.created_at)}</span></p>
...
function renderPagination(podcastId) {
  ...
  if (
    existingPagination &&
    existingPagination.classList.contains("comment-pagination")
  ) {
  ...
  if (currentPage > 1) {
    buttons += `<button class="page-btn prev" data-page="${
      currentPage - 1
    }">Previous</button>`;
  }
  ...
  if (currentPage < totalPages) {
    buttons += `<button class="page-btn next" data-page="${
      currentPage + 1
    }">Next</button>`;
  }

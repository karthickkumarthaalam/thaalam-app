...
function renderPackages() {
  ...
  items.forEach((pkg, index) => {
    ...
    let priceLabel =
      selectedDuration === "yearly" && pkg.yearly_price
        ? `<div class="yearly-price">
                        <span class="old-price">${
                          pkg.symbol || "CHF"
                        } ${pkg.price.toFixed(2)} / month</span>
                        <span>${pkg.symbol || "CHF"} ${pkg.yearly_price.toFixed(
            2
          )} / month</span>
                    </div>`
        : `<span>${pkg.symbol || "CHF"} ${pkg.price.toFixed(2)} / ${
            pkg.duration
          }</span>`;
                ...
                <div class="accordion-item ${index === 0 ? "active" : ""}">
                    <div class="accordion-header">
                        ...
                        <div class="plan-info">
                            ...
                            <p>${
                              pkg.description || "Thaalam Membership Plan"
                            }</p>
                        </div>
...
async function buyPackage(packageId) {
  ...
  const unitPrice = isYearly
    ? packageDetails.yearly_price.toFixed(2)
    : packageDetails.price.toFixed(2);
  const totalPrice = isYearly
    ? (packageDetails.yearly_price * 12).toFixed(2)
    : packageDetails.price;
  const durationLabel = isYearly
    ? "per Year"
    : `per ${packageDetails.duration}`;
    ...
    ">
      ...
      ">
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
            ${
              durationLabel !== "free"
                ? `<span style="font-size:15px; color:#555;">Billing Cycle</span>
  ...
  if (result.isConfirmed) {
    try {
      const paymentIntentRes = await fetch(
        `${window.API_BASE_URL}/payments/initiate`,
        {
          method: "POST",
          headers: {
    ...
    ">
      ...
      <p style="font-size:15px; color:#444; margin:0;">
        ${
          data.message ||
          "Unable to initiate payment. Please verify your details and try again."
        }
      </p>
    </div>
  `,
